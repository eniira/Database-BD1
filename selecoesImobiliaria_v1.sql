...
-- 1 Selecione os nomes dos mercadólogos que fizeram o anuncios para imoveis de alugueis.
SELECT F.nome, F.cpf
FROM FUNCIONARIO_MERCADOLOGO M, FUNCIONARIO F, MERC_FAZ_ANUN MA
WHERE F.cpf = M.cpf_merc 
AND M.cpf_merc = MA.cpf_merc 
AND MA.codigo IN (SELECT A.codigo FROM ANUNCIO A, IMOVEL I, IMOVEL_ALUGUEL IA WHERE  I.codigo = IA.codigo_alug AND I.codigo = A.codigo_imo);

-- 2 Selecione os nomes dos inquilinos que tem fiadores invalidos: 
SELECT N.nome
FROM NEGOCIADOR N, NEGOCIADOR_INQUILINO NI
WHERE N.email = NI.email
AND NI.cpf_fia IN (SELECT F.cpf FROM FIADOR F WHERE F.contrato = 'inválido'); 

-- 3 Selecione os clientes, sem repetição, que tem apenas imóveis com status out:
SELECT DISTINCT C.nome
FROM CLIENTE C, IMOVEL I 
WHERE C.email = I.email_cli
AND C.nome NOT IN (SELECT C1.NOME FROM CLIENTE C1, IMOVEL I1 WHERE C1.email = I1.email_cli AND I1.stats <> 'out');

-- 4 Selecione o código de imóveis de venda mais caro:
SELECT I.codigo 
FROM IMOVEL I, IMOVEL_VENDA IV 
WHERE I.codigo = IV.codigo_vend 
AND IV.valorTotal >= ALL (SELECT IV1.valorTotal FROM IMOVEL_VENDA IV1);

-- 5 Selecione os clientes que tem imóveis com status in e out: 
SELECT DISTINCT C.nome
FROM CLIENTE C, IMOVEL I 
WHERE C.email = I.email_cli
AND I.stats = 'in' 
AND C.nome IN (SELECT C1.nome FROM CLIENTE C1, IMOVEL I1 WHERE C1.email = I1.email_cli AND I1.stats = 'out');

-- 6 Selecione os clientes que vendem casas ou aps: 
SELECT C.nome 
FROM CLIENTE C, IMOVEL I, IMOVEL_VENDA IV 
WHERE C.email = I.email_cli
AND I.codigo = IV.codigo_vend
AND IV.tipo = 'casa'
UNION 
SELECT C1.nome 
FROM CLIENTE C1, IMOVEL I1, IMOVEL_VENDA IV1 
WHERE C1.email = I1.email_cli
AND I1.codigo = IV1.codigo_vend
AND IV1.tipo = 'ap';

-- 7 Selecione os inquilinos em que a dataFim é em 2021:
SELECT I.codigo 
FROM IMOVEL I
WHERE I.codigo IN (SELECT I1.codigo FROM IMOVEL I1, IMOVEL_ALUGUEL IA WHERE I1.codigo = IA.codigo_alug AND IA.dataFim LIKE '2021%');

-- 8 Selecione os imóveis que foram anunciados mais vezes do que os que já foram vendidos. 
SELECT I.codigo  
FROM IMOVEL I, ANUNCIO A
WHERE I.codigo = A.codigo_imo
AND A.vzs_anuncio > ALL (SELECT A1.vzs_anuncio FROM IMOVEL I1, ANUNCIO A1 WHERE I1.codigo = A1.codigo_imo AND I1.stats = 'out');

-- 9 Selecione, sem repetição, o tipo de imovel que custa menos que 300.000:
SELECT  DISTINCT IV.tipo, IV.valorTotal
FROM IMOVEL_VENDA IV
WHERE IV.codigo_vend  IN (SELECT IV1.codigo_vend FROM IMOVEL_VENDA IV1 WHERE IV1.valorTotal <  300000.00 );

-- 10 Selecione se o corretor de Joana Santos vendeu o imóvel dela, ou seja, se o stats = 'in', não vendeu e se for 'out', vendeu:
SELECT I.stats 
FROM IMOVEL I, IMOVEL_VENDA IV
WHERE I.codigo = IV.codigo_vend
AND IV.cpf_corr IN (SELECT CTC.cpf_corr FROM CORR_TEM_CLI CTC, CLIENTE C WHERE CTC.email_cli  =  C.email AND C.nome = 'Joana Santos');
